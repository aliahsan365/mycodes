...
void BoundingBox::postFrame()
{
  ...
  program->setUniformValue("mvp",
                           ...
                           camera()->viewMatrix());
  for (unsigned int i = 0; i <int(scene()->objects().size()); ++i ) {
...
*/
  ...
  //vertices e indices ya incluye el codigo para hacer el clear
  vertices.clear();
  indices.clear();
}
...
void BoundingBox::createBox(unsigned int i)
{
    Box   bBox = scene()->objects()[i].boundingBox();
