...
writeClauses(MaxCost):-
    ...
    teamSPlaysAgainstTeamTExactlyOnce,
    atLeastMovistarHasOneTvMatch,
    
    matchSTimpliesHomeS,
    matchSTimpliesAwayT,
    
    teamSDoesNotPlayAtHomeCertainRound,
    
    
    sPlaysHomeInRSPplaysHomeRplus1thenDoubleInR,
    shomeSawayImpliesNoDouble,
    sawayshomeimpliesnodouble,
    sawaysawayimpliesdouble,
    eachTeamNoDoublesInRoundR,
    
    
    triplesAwayAreNotAllowed,
    triplesAtHomeAreNotAllowed,
...
teamSPlaysAgainstTeamTExactlyOnce:- team(S),
									team(T),
									S\=T,
...
atLeastMovistarHasOneTvMatch:- round(R),
							   findall(match-S-T-R,(team(S),team(T),T\=S,tvMatch(S,T)),Lits1),
							   findall(match-T-S-R,(team(T),team(S),T\=S,tvMatch(T,S)),Lits2),
							   append(Lits1,Lits2,Lits),
							   atLeast(1,Lits),
							   fail.
atLeastMovistarHasOneTvMatch.
...
teamSDoesNotPlayAtHomeCertainRound:- team(S),
								 notHome(S,L),
								 member(R,L),
...
sPlaysHomeInRSPplaysHomeRplus1thenDoubleInR:- team(T),
											  round(R1),
											  round(R2),
											  R2 is R1+1,
...
shomeSawayImpliesNoDouble:- team(T),
						    round(R1),
							round(R2),
							R2 is R1+1,
							writeClause([\+home-T-R1, home-T-R2, \+double-T-R2]),
...
sawayshomeimpliesnodouble:- team(T),
							round(R1),
							round(R2),
							R2 is R1+1,
							writeClause([home-T-R1, \+home-T-R2, \+double-T-R2]),
...
sawaysawayimpliesdouble:-     team(T),
							  round(R1),
							  round(R2),
							  R2 is R1+1,
							  writeClause([home-T-R1, home-T-R2,double-T-R2]),
...
eachTeamNoDoublesInRoundR:- noDoubles(L), 
							team(S),
							member(R,L),
...
%%%%%%tirples no permitidos%%%

triplesAwayAreNotAllowed:- team(S),
							 round(R1),
							 round(R2),
							 round(R3),
							 R2 is R1 + 1,
							 R3 is R1 + 2, 
							 writeClause([home-S-R1,home-S-R2,home-S-R3]),
							 fail.
triplesAwayAreNotAllowed.


triplesAtHomeAreNotAllowed:- team(S),
							 round(R1),
							 round(R2),
							 round(R3),
							 R2 is R1 + 1,
