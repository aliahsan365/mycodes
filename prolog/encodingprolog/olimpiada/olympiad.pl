...
compatible(S1,S2):- student(S1), 
					student(S2),
					friends(S1,S2),friends(S2,S1),
					expertise(S1,E1),
					expertise(S2,E2),
					E1 \= E2.
...
writeClauses:-
	noincompatiblechosen,
	exactlyKMembers,
	true.
    
    
exactlyKMembers:- numMembersTeam(K),findall(chosen-S,student(S),Lits),exactly(K,Lits), fail.
exactlyKMembers.

noincompatiblechosen:- student(S1), student(S2), S1  \= S2, \+compatible(S1,S2), writeClause([\+chosen-S1,\+chosen-S2]),fail. 
noincompatiblechosen.
