...
writeClauses:-
	eachhouratleastMnurses,
	nurseNdoesnotworkhoursH,
	eachnurseexactlyonetype,
	eachnursestartsexactlyonce,
	eachnurseeachtypeeachstarthworksnurseNhourH,
	eachnurseeachtypeeachstarthNotworksnurseNhourH,
	true.
 
 %mira si per cada hora tenim com a minim M nurses
 eachhouratleastMnurses:- hour(H),needed(H,M),findall(worksNH-N-H,nurse(N),Lits), atLeast(M,Lits),fail.
 eachhouratleastMnurses. 

 %bloqueig dhores per cada nurse(bolqueig nomes de les que no pot treballar)
 nurseNdoesnotworkhoursH:-nurse(N),nurseIDandBlocking(N,HS,HE),hourInRange(HS,HE,H), writeClause([\+worksNH-N-H]),fail.
 nurseNdoesnotworkhoursH.
 
 %nurse n numes por ser dun sol tipos (exactament)
 eachnurseexactlyonetype:- nurse(N),findall(nurseType-N-Type,type(Type),Lits),exactly(1,Lits),fail.
 eachnurseexactlyonetype.
 
 %cada nurse nomes comenza un cop.
 eachnursestartsexactlyonce:-nurse(N),findall(startsNH-N-H,hour(H),Lits), exactly(1,Lits),fail.
 eachnursestartsexactlyonce.
 
 %starts && type -> works.
 eachnurseeachtypeeachstarthworksnurseNhourH:- nurse(N),hour(StartH), type(Type), workingHourForTypeAndStartH(Type,StartH,H), writeClause([\+startsNH-N-StartH,\+nurseType-N-Type,worksNH-N-H]),fail.
 eachnurseeachtypeeachstarthworksnurseNhourH.
 
 eachnurseeachtypeeachstarthNotworksnurseNhourH:- nurse(N), hour(H), hour(StartH), type(Type), not(workingHourForTypeAndStartH(Type,StartH,H)), writeClause([\+startsNH-N-StartH,\+nurseType-N-Type,\+worksNH-N-H]), fail.
 eachnurseeachtypeeachstarthNotworksnurseNhourH.
