...
family(g01,f1). %family 1
family(g02,f1). 
family(g03,f1).
family(g04,f2). %family 2
family(g05,f2).
family(g06,f2).
family(g07,f3). %family 3
family(g08,f3).
family(g09,f3).
family(g10,f4). %family 4
family(g11,f4).
family(g12,f4).

familiadelgangster(G,F):-gangster(G),family(G,F).

familia(F):- familias(L), member(F,L).
...
%   3. involved-F-T means :  "si indican que la familia F está involucrada en la tarea T"
...
maxConsecutiveHours(M,K):- 
	...
	gangster(G), 
	task(T),
	findall(H,member(does-G-T-H,M),Hours),
	sort(Hours,SortedHours),
	
	append(_,L1,SortedHours),
	append(FHours,_,L1),
...
writeClauses(K):- 
	...
	ifdoesthenwork,
    nokconsecutivas(K),
    exactamenteNgangsters,
    siguientehoranodiferentetarea,
    gangsterbloqueados,
    
    comomuchosdostareas,
    implicadoporquetrabaja,
    true,!.
...
comomuchosdostareas:-
	familia(F),
	task(T),
    findall(involved-F-T,task(T),Lits),
    atMost(2,Lits),
    fail.
comomuchosdostareas.
...
implicadoporquetrabaja:-
    ...
    familia(F),
    hour(H),
    writeClause([\+does-G-T-H,involved-F-T]),
    fail.
implicadoporquetrabaja.
...
nokconsecutivas(K) :-
    ...
    findall(\+works-G-HR, ventanatiempo(H, K, HR), Lits),
    length(Lits, S),
    S > 0,
    writeClause(Lits),
    fail.
...
displaySol(M):- nl,nl,
    ...
    familia(F),write('Family '), write(F),write(': '),task(T1),task(T2), write(T1), write(' '), write(T2),nl,fail.
