...
%bien
eachCityAtLeastOneAntena:-city(C,Antenas),findall(covering-C-I,member(I,Antenas),L),atLeast(1,L),fail.
eachCityAtLeastOneAntena.
...
displaySol(M):- findall(used-I,member(used-I,M),L),sort(L,O),writeList(O).
displaySol(_).
