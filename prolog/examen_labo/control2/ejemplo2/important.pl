...
%covering-C-I : means : Antena I is covering City C.
%used-I : means : Antena I is used


antena(I):- numStations(N),between(1,N,I).


writeClauses(K):- 
	eachCityAtLeastOneAntena,
	usedImpliesCoveringAny,
	maxKAntenaUsed(K),
eachCityAtLeastOneAntena2,
	true,!.  

%bien
eachCityAtLeastOneAntena:-city(C,Antenas),not(member(C,[1,5,10,20])),findall(covering-C-I,member(I,Antenas),L),atLeast(1,L),fail.
eachCityAtLeastOneAntena.

eachCityAtLeastOneAntena2:-city(C,Antenas),member(C,[1,5,10,20]),findall(covering-C-I,member(I,Antenas),L),atLeast(2,L),fail.
eachCityAtLeastOneAntena2.

usedImpliesCoveringAny:- antena(I),findall(covering-C-I,city(C,_),Lits),expressOr(used-I,Lits),fail.
usedImpliesCoveringAny.

maxKAntenaUsed(K):- findall(used-I,antena(I),Lits),atMost(K,Lits),fail.
maxKAntenaUsed(_).
...
%bien
cost(M,Cost):- findall(used-I,member(used-I,M),L),length(L,Cost),true.

%Show the solution:	

writeList([]).
writeList([used-I|L]):-write(I),write(' '),writeList(L).

displaySol(M):- findall(used-I,member(used-I,M),L),sort(L,O),writeList(O),fail.
displaySol(_).
